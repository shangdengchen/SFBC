...
OneButton::OneButton(const int pin, const bool activeLow, const bool pullupActive) {
    // 根据参数配置按钮引脚的模式和行为：
    // - pin：按钮连接的引脚编号。
    // - pullupActive：是否启用内部上拉电阻。
    //   - 如果为 true，则将引脚模式设置为 INPUT_PULLUP，表示启用内部上拉电阻。
    //   - 如果为 false，则将引脚模式设置为 INPUT，表示不启用内部上拉电阻。
    // - activeLow：按钮的电平极性：
    //   - 如果为 true，表示按钮按下时引脚电平为低（低电平有效）。
    //   - 如果为 false，表示按钮按下时引脚电平为高（高电平有效）。
