...
// - 适用于BLDC电机的3PWM设置
// - 硬件特定的函数
// 支持Arduino/ATmega328、STM32和ESP32
void* _configure3PWM(long pwm_frequency, const int pinA, const int pinB, const int pinC) {
    // 如果未设置PWM频率，则使用默认频率25kHz
    if (!pwm_frequency || !_isset(pwm_frequency))
        pwm_frequency = _PWM_FREQUENCY; // 默认频率为25kHz
        // 限制PWM频率的最大值为40kHz
    else
        pwm_frequency = _constrain(pwm_frequency, 0, _PWM_FREQUENCY_MAX); // 限制最大频率为40kHz

    // 定义一个BLDC电机3PWM槽结构体
    bldc_3pwm_motor_slots_t m_slot = {};

    // 确定连接的是哪个电机，并设置相应的配置参数
    int slot_num;
    for (slot_num = 0; slot_num < 4; slot_num++) {
        // 将新电机放在第一个空闲槽中
        if (esp32_bldc_3pwm_motor_slots[slot_num].pinA == _EMPTY_SLOT) {
            esp32_bldc_3pwm_motor_slots[slot_num].pinA = pinA; // 设置槽的pinA为当前电机的pinA
            m_slot = esp32_bldc_3pwm_motor_slots[slot_num]; // 将当前槽的配置信息存储到m_slot中
            break;
        }
    }

    // 禁用与当前电机使用相同MCPWM的其他槽
    // 禁用可能会占用相同槽的2PWM步进电机
    esp32_stepper_2pwm_motor_slots[slot_num].pin1pwm = _TAKEN_SLOT; // 标记该槽已被占用
    ...
    // 配置引脚
    mcpwm_gpio_init(m_slot.mcpwm_unit, m_slot.mcpwm_a, pinA); // 初始化pinA对应的MCPWM引脚
    mcpwm_gpio_init(m_slot.mcpwm_unit, m_slot.mcpwm_b, pinB); // 初始化pinB对应的MCPWM引脚
    mcpwm_gpio_init(m_slot.mcpwm_unit, m_slot.mcpwm_c, pinC); // 初始化pinC对应的MCPWM引脚

    // 配置定时器频率
    _configureTimerFrequency(pwm_frequency, m_slot.mcpwm_num, m_slot.mcpwm_unit); // 根据PWM频率配置定时器

    // 创建一个ESP32MCPWMDriverParams结构体
    ESP32MCPWMDriverParams* params = new ESP32MCPWMDriverParams {
            .pwm_frequency = pwm_frequency, // 存储PWM频率
            .mcpwm_dev = m_slot.mcpwm_num, // 存储MCPWM设备编号
            .mcpwm_unit = m_slot.mcpwm_unit, // 存储MCPWM单元编号
            .mcpwm_operator1 = m_slot.mcpwm_operator // 存储MCPWM操作器编号
    };
    return params; // 返回指向该结构体的指针
}
